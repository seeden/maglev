...
/**
 * Enable secured name of files
 * @param  {[type]}   req  [description]
 * @param  {[type]}   res  [description]
 * @param  {Function} next [description]
 * @return {[type]}        [description]
 */
exports.securedName = function(req, res, next) {
	req.params.securedName = true;
	next();
};

exports.storeFirstImage = function(options) {
	options = options || {};
	options.exts = options.exts || ['jpg', 'jpeg', 'png'];
	...
	return function(req, res, next) {
		...
		var file = files[0];
		var orgPath = file.path;
		var image = gm(orgPath);
		if(!image) {
		...
		image.size(function (err, size) {
			...
			if(!changed) {
				return storeFirst(req, res, next);
			}
...
var storeFirst = exports.storeFirst = function(req, res, next) {
		var config = req.server.config;
		var files = req.objects.files;
		...
		File.createFile(file, config.upload.path, {}, req.params.securedName, function(err, file) {
			if(err) {
				return next(err);
			}
