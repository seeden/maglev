...
exports.ensureBySignedRequest = function(req, res, next) {
    if(!req.body.signed_request || !req.body.profile) return res.jsonp(400, {
    ...
    if(!signedRequest) return res.jsonp(400, {
            error: 'Parsing signed request'
        });

    if(!signedRequest.user_id) return res.jsonp(400, {
            error: 'User ID error'
        });
    ...
    User.findByFacebookID(signedRequest.user_id, function(err, user) {
        ...
        if(user) return req.logIn(user, {session: session}, next);

        if(!config.registration.simple) return res.jsonp(400, {
