...
exports.remove = function(req, res, next) {
	...
	rbac.removeByName(req.body.permission, function(err, isDeleted) {
		if(err) {
			return next(err);
		}
...
exports.exists = function(req, res, next) {
	...
	if(!req.body.permission) {
		return next(new WebError(400, 'Permission is undefined'));
	}

	rbac.existsPermission(req.body.permission, function(err, exists) {
		if(err) {
			return next(err);
		}
...
exports.get = function(req, res, next) {
	...
	rbac.getPermission(req.body.permission, function(err, permission) {
		if(err) {
			return next(err);
		}
