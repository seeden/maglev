...
exports.change = function(req, res, next) {
	...
	if(!req.body.password) return res.jsonp(400, {
	...
	if(!user.hasPassword()) {
		...
		if(!req.body.password_old) return res.jsonp(400, {
		...
		user.comparePassword(req.body.password_old, function(err, isMatch) {
			...
			if(!isMatch) return res.jsonp(400, {
...
exports.forgot = function(req, res, next) {
	...
	if(!user.hasEmail()) return res.jsonp(400, {
...
exports.forgotResponse = function(req, res, next) {
	...
	if(!req.query.token) return res.jsonp(400, {
	...
	jwt.verify(req.query.token, config.mail.token.secret, function(err, data) {
		...
		if(!data.userID) return res.jsonp(400, {
				error: 'Unknown user'
			});	
