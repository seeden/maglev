...
req.can = function(action, resource) {
	...
	if(this.isGuest() === true) {
		var roleGuest = rbac.getRole(config.rbac.roleGuest);
		if(!roleGuest) {
			return false;	
		}

		return roleGuest.can(action, resource);
	}

	//process user
	return this.user.can(rbac, action, resource);
};

req.hasRole = function(name) {
	var server = this.server;
	var config = server.config;
	var rbac = server.rbac;


	if(!this.isGuest() && this.user.hasRole(name)) {
		return true;
	}
