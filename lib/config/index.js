...
Config.prototype.validate = function() {
	if(!this.server.root) throw new Error('Server root directory is undefined.');
	if(!this.db.uri) throw new Error('Database uri is undefined.');
	if(!this.token || !this.token.secret) throw new Error('Token secret is undefined. Please see configuration.');
	if(!this.session || !this.session.secret) throw new Error('Session secret is undefined. Please see configuration.');
//	if(!this.mail || !this.mail.options) throw new Error('Mail options is undefined.');
};
