...
function genUniqueName(securedName, cb) {
	...
	if(securedName) {
		unique = puid.generate() + '-' + uuid.v4();
	} else {
		unique = puidShort.generate();
	}
...
var createFile = exports.createFile = function(file, basePath, data, securedName, cb) {
	var File = this.model(name),
		originalFilename = file['originalFilename'];

	genUniqueName(securedName, function(err, name) {
		if(err) {
			return cb(err);
		}
