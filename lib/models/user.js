...
var createByFacebook = exports.createByFacebook = function(profile, cb) {
	...
	var data = {
		...
		providers: [{
			name: 'facebook',
			uid: profile.id,
			name_uid: provider.genNameUID('facebook', profile.id),
			data: profile
		}]
	};
...
var generateAccessToken = exports.generateAccessToken = function(tokenSecret, expiresInMinutes, scope) {
	if(!tokenSecret) throw new Error('Token secret is undefined');
...
var findByProviderUID = exports.findByProviderUID = function(providerName, uid, cb) {
	this.findOne({
		'providers.name_uid': provider.genNameUID(providerName, uid)
	}).exec(cb);
};
 ...
 * @param  {Function} cb
 */
var findByUsernamePassword = exports.findByUsernamePassword = function(username, password, cb) {
	this.findOne({ $or: [ 
		{ username: username }, 
		{ email: username } 
		...
		user.comparePassword(password, function(err, isMatch) {
			...
			if(!isMatch) return cb(new Error('Invalid password'));
			cb(null, user);
		});
	});
};
 ...
 */
var hasRole = exports.hasRole = function(role, cb) {
 ...
 * @param  {String|Array}   roles  Array of roles or string representing of role
 * @param  {Function} cb Callback
 */
var addRoles = exports.addRoles = function(roles, cb) {
	var self = this;

	roles = typeof roles === 'string' ? [roles] : roles;

	this.update({$addToSet: {roles: {$each: roles}}}, function(err, obj) {
		if(err) return cb(err);
...
var getProvider = exports.getProvider = function(providerName) {
...
var hasProvider = exports.hasProvider = function(providerName) {
...
var getProvidersNameUIDs = exports.getProvidersNameUIDs = function() {
	...
	for(var i=0; i<this.providers.length; i++) {
		var provider = this.providers[i];
		providers.push(provider.name_uid);
	}
...
/*
	...
	schema.index({'providers.name_uid': 1}, {unique: true});
